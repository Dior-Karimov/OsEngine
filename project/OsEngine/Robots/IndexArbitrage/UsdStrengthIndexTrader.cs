    ...
    public class UsdStrengthIndexTrader : BotPanel
    {
        ...
        private void EvaluateClose(BotTabSimple tab, List<Candle> candles)
        {
            ...
            for (int i = 0; i < positions.Count; i++)
            {
                ...
                string riskExitReason = GetRiskExitReason(position, tab, lastTrade.Close, candles[candles.Count - 1].TimeStart);
                if (string.IsNullOrEmpty(riskExitReason) == false)
                {
                    tab.CloseAtMarket(position, position.OpenVolume, riskExitReason);
                    continue;
                }
        ...
        private string GetRiskExitReason(Position position, BotTabSimple tab, decimal lastClose, DateTime now)
        {
            ...
            if (_stopLossPercent.ValueDecimal > 0 && changePercent <= -_stopLossPercent.ValueDecimal)
            {
                return "RiskStopLoss";
            }

            if (_takeProfitPercent.ValueDecimal > 0 && changePercent >= _takeProfitPercent.ValueDecimal)
            {
                return "RiskTakeProfit";
            }
